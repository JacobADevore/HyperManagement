...
class DeleteLock
{
     ...
     * @param $jobTitleName
     */
    public function __construct($adminId, $userId, $userJobTitle, $lockName)
    {
        ...
        if ($this->requirementCheckLockName($adminId, $lockName) == false) {
            return;
        }

        // upload job to 'JobTitles'
        if ($this->deleteLock() == false) {
            return;
        }
    ...
    public function requirementCheckLockName($adminId, $lockName) {
        // datastore connection
        $datastore = new DatastoreClient();
        ...
        if ($count != 1) {
            // returns error alert
            $this->alert['error'] = 'Invalid lock, we could\'t find a '.$lockName.' under your account locks list. please refresh and try again';
            return false;
        }
    ...
    public function deleteLock() {
        ...
        $result = $datastore->delete($datastore->key('Locks', $this->datastoreLockId));
