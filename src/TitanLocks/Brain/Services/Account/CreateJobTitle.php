...
class CreateJobTitle
{
     ...
     * @param $jobTitleName
     */
    public function __construct($adminId, $userId, $userJobTitle, $jobTitleName, $features)
    {
    ...
    public function checkJobTitleName($jobTitleName, $userId) {
        ...
        $jobTitleRequirementCheck = RequirementCheck::jobTitleName($jobTitleName);
        ...
        if ($count == 1) {
            // returns error alert
            $this->alert['error'] = 'Job title name already in use, please change the name and try again';
            return false;
        }
    ...
    public function uploadJobTitle($userId, $jobTitleName, $features) {
        ...
        $emailVerificationInfo = [
            'name' => ucfirst($jobTitleName),
            'adminUserId' => $userId,
            'features' => $features
        ];

        // create entity for datastore
        $emailVerification = $datastore->entity($datastore->key('JobTitles'), $emailVerificationInfo);
        ...
        // sends the datastore result to datastore ErrorHandling
        $datastoreErrorHandling = Datastore::ErrorHandling($result, 'CreateJobTitle::uploadJobTitle', 'datastore');
        // checks if datastore error handling caught any errors
        if ($datastoreErrorHandling != false) {
            // returns error alert
            $this->alert['error'] = $datastoreErrorHandling;
