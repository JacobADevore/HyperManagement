...
class DeleteEkey
{
     ...
     * @param $ttlPassword
     */
    public function __construct($adminId, $userId, $userJobTitle, $eKeyId, $ttlUsername, $ttlPassword)
    {
        ...
        if ($this->deletePasscode($eKeyId, $ttlUsername, $ttlPassword) == false) {
            return;
        }
    ...
    // upload job to 'JobTitles'
    public function deletePasscode($eKeyId, $ttlUsername, $ttlPassword) {
        ...
        $deletePasscodeURL = 'https://api.sciener.cn/v3/key/delete';

        try {
            $deletePasscodeResponse = $client->request('POST', $deletePasscodeURL, $deletePasscode);
        } catch (RequestException $e) {
