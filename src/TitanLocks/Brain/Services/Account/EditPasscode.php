...
class EditPasscode
{
    ...
    public function __construct($adminId, $userId, $userJobTitle, $editPasscode, $editStartTime, $editEndTime, $passcodeId, $lockId, $ttlUsername, $ttlPassword)
    {
        ...
        if ($this->requirementCheckInfo($adminId, $editPasscode, $editStartTime, $editEndTime) == false) {
            return;
        }

        // create the location name into the data store
        if ($this->editPasscode($adminId, $editPasscode, $editStartTime, $editEndTime, $passcodeId, $lockId, $ttlUsername, $ttlPassword) == false) {
            return;
        }
    ...
    // requirement check location name and make sure a location name isn't already created under that name
    public function requirementCheckInfo($adminId, $editPasscode, $editStartTime, $editEndTime) {
        // requirement check job title name
        $jobTitleRequirementCheck = RequirementCheck::passcode($editPasscode);
        ...
        if ($editStartTime != '' || $editEndTime != '' && round(microtime(true) * 1000) > (strtotime($editStartTime) * 1000) && (strtotime($editStartTime) * 1000) > (strtotime($editEndTime) * 1000)) {
            // returns error alert
            $this->alert['error'] = 'Start time or End time can\'t be in the past and start time and end time can\'t be set individually ( if one is set the other must be too ) as well as End time must be equal to or greater than the Start time, please try again';
            return false;
        }
    ...
    // create the location name into the data store
    public function editPasscode($adminId, $editPasscode, $editStartTime, $editEndTime, $passcodeId, $lockId, $ttlUsername, $ttlPassword) {
        ...
        $lockListUrl = 'https://api.sciener.cn/v3/keyboardPwd/change';
